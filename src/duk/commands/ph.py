...
class PriceHistoryDownloader:
    ...
    PRICE_HISTORY_URL = "https://financialmodelingprep.com/stable/historical-price-eod/full"
    ...
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "duk-price-history-downloader/0.1.0"})
    ...
    def download_price_data(
        ...
        try:
            ...
            logger.info(f"Downloading price data for {symbol} from {start_date} to {end_date}")
    ...
    def _filter_by_date_range(
        self, data: List[Dict[str, Any]], start_date: Optional[str], end_date: Optional[str]
    ) -> List[Dict[str, Any]]:
...
def process_price_data(
    ...
    Args:
        ...
        frequency: Data frequency (daily, weekly, monthly, quarterly, semiannual, annual)
...
@click.option(
    "--filename",
    "-f",
    help="Specify filename (overrides default naming)"
)
@click.option(
    "--format",
    "output_format",
...
@click.option(
    "--frequency",
    type=click.Choice(["daily", "weekly", "monthly", "quarterly", "semiannual", "annual"]),
    default="daily",
...
@click.option(
    "--ohlc",
    is_flag=True,
    help="Include Open, High, Low, Close prices"
)
@click.option(
    "--hlc",
    is_flag=True,
    help="Include High, Low, Close prices"
)
@click.option(
    "--ohlcv",
    is_flag=True,
    help="Include Open, High, Low, Close, Volume"
)
@click.option(
    "--vol",
    is_flag=True,
...
@click.option(
    "--adj",
    is_flag=True,
    help="Include adjusted close prices"
)
@click.option(
    "--div",
    is_flag=True,
    help="Include dividend data"
)
@click.option(
    "--split",
    is_flag=True,
    help="Include stock split data"
)
@click.pass_context
def ph_command(
    ctx,
    ...
    TICKER can be either a single ticker symbol or a path to a file containing ticker symbols.
    
    By default, returns the 5 most recent OHLC data points for the specified ticker(s).
    ...
    if start_date and end_date and days:
        if days != 5:  # Only error if user explicitly set days different from default
            click.echo(
                "Error: Cannot specify --days with both --start-date and --end-date", err=True
            )
            sys.exit(1)
    ...
    for symbol in tickers:
        ...
        dividends_data = None
        if div or adj:
            dividends_data = downloader.download_dividends_data(symbol, start_date, end_date)
