...
class TestDistributionBuilding:
    ...
    def test_make_dist_builds_packages(self):
        ...
        if result.returncode != 0:
            pytest.fail(f"make dist failed with exit code {result.returncode}:\n"
        ...
        assert len(dist_files) >= 2, f"Expected at least 2 files in dist/, got {len(dist_files)}"
        ...
        assert "duk-0.1.0" in wheel_file.name, f"Wheel file name doesn't contain version: {wheel_file.name}"
        assert "duk-0.1.0" in tar_file.name, f"Tar file name doesn't contain version: {tar_file.name}"
